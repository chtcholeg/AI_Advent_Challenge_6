...
class ChatRepositoryImpl(
    ...
    override suspend fun sendMessage(userMessage: String): String {
        ...
        if (settings.useStructuredResponse) {
            if (currentSystemPrompt != settings.systemPrompt) {
                // System prompt changed, clear history and add new system message
                conversationHistory.clear()
                conversationHistory.add(Message(role = "system", content = settings.systemPrompt))
                currentSystemPrompt = settings.systemPrompt
            }
        } else {
            // Structured response disabled, remove system prompt
