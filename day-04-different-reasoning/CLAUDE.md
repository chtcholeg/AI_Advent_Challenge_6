...
Response modes (Normal, JSON, Dialog, Step-by-Step, Expert Panel) are mutually exclusive through dropdown menu selection.
...
- `domain/model/ResponseMode`: Enum for AI response modes (NORMAL, STRUCTURED_JSON, DIALOG, STEP_BY_STEP, EXPERT_PANEL) - mutually exclusive by design
- `domain/model/AiSettings`: AI parameter configuration with validation (temperature, topP, maxTokens, repetitionPenalty, responseMode, systemPrompt). Includes system prompts: DIALOG_SYSTEM_PROMPT, STEP_BY_STEP_SYSTEM_PROMPT, EXPERT_PANEL_SYSTEM_PROMPT
