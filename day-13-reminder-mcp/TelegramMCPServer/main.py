...
Usage:
    python main.py                # with auth (requires MCP_API_KEY env var)
    python main.py --no-auth      # without authentication
...
Setup (run once before starting the server):
    ...
    2. Run `python setup_session.py` to authenticate and create session file
    3. Start the server: `python main.py --no-auth`
"""
...
def build_app(config, telegram_client: TelegramChannelClient) -> FastAPI:
    ...
    protocol_handler = McpProtocolHandler(tools, server_name="telegram-mcp-server", server_version="1.0.0")
...
def main():
    ...
    logger.info("Telegram MCP Server v1.0.0")
    ...
    logger.info("  Tools:        get_channel_messages, get_channel_info")
