# GigaChat Multiplatform Chat Application (Day 14 - MCP Composition)
...
## Updates in Day 14: MCP Composition

This version introduces **MCP Composition** - a comprehensive ecosystem of 6 MCP servers working together, plus a new AI Agent application module. This release expands from 2 servers to 6 specialized servers with centralized management, demonstrating the full power of Model Context Protocol integration.

### New Features

#### MCP Server Collection (6 Servers)

The project now includes a comprehensive collection of MCP servers:

1. **GitHub MCP** (port 8000) - Repository data and operations (existing)
2. **Telegram MCP** (port 8001) - Channel reader with monitoring (existing)
3. **Weather MCP** (port 8002) - Current weather & forecasts via Open-Meteo (NEW)
4. **TimeService MCP** (port 8003) - Time in any timezone/city (NEW)
5. **Currency MCP** (port 8004) - Exchange rates & conversion via ECB (NEW)
6. **FileOps MCP** (port 8005) - Secure file system operations (NEW)
...
python launcher.py --all --no-auth    # Start all 6 servers
...
# Start all 6 servers at once
python launcher.py --all --no-auth
```

Servers will start on ports 8000-8005. Add them in app Settings → MCP Servers:
...
python launcher.py weather timeservice currency
